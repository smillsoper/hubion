...
export interface HubionNodeDef {
  type: HubionNodeType
  label: string
  content?: string
  fieldType?: string
...
export interface HubionFlowDefinition {
  flow_type: 'crm' | 'telephony'
  name: string
  entry_node: string
  nodes: Record<string, HubionNodeDef>
}
...
export function defaultNodeData(type: HubionNodeType): NodeData {
  switch (type) {
    case 'script':
